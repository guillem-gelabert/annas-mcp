...
- ✅ **v1.4 DOI Verification** — Phases 15–17 (shipped 2026-05-27)
...
### v1.4 DOI Verification (Phases 15–17)

- [x] **Phase 15: CrossRef Client** — fetch canonical title from CrossRef with proper User-Agent and timeout (completed 2026-05-27)
- [x] **Phase 16: Confidence Logic** — Jaccard similarity comparison producing high/low/unverified confidence (completed 2026-05-27)
- [x] **Phase 17: Response Integration** — wire verification into single-DOI and batch handlers with full backwards compatibility (completed 2026-05-27)
...
**Success Criteria** (what must be TRUE):
  ...
  2. The outgoing request carries a descriptive `User-Agent` header (e.g. `annas-mcp-ts/1.4 (mailto:...)`)
...
**Plans**: 1 plan
Plans:
- [x] 15-01-PLAN.md — Implement fetchCrossRefTitle with TDD (failing tests then implementation)
...
**Plans**: 1 plan
Plans:
- [x] 16-01-PLAN.md — Implement computeConfidence with TDD (failing tests then implementation)

### Phase 17: Response Integration
**Goal**: Every `article_download` response includes a `verification` object, and no existing behaviour changes
**Depends on**: Phase 16
**Requirements**: VER-07, VER-08, VER-09, VER-10, VER-11
**Success Criteria** (what must be TRUE):
  1. Single-DOI `article_download` response contains `verification: { crossrefTitle, annasTitle, confidence }` alongside all existing fields
  2. Batch `article_download` response contains the same `verification` shape on each per-article result
  3. When CrossRef errors or times out, the tool still returns sources with `confidence: "unverified"` — no hard failure
  4. All 95 existing tests continue to pass; new verification tests use a mock CrossRef fetch
**Plans**: 1 plan
Plans:
- [x] 17-01-PLAN.md — Wire verification into single-DOI and batch handlers (TDD)
...
---
*Last updated: 2026-05-27 — Phase 17 planned (1 plan)*
