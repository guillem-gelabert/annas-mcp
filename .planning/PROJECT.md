...
- ✅ **v1.4 DOI Verification** — Phases 15–17 (shipped 2026-05-27): CrossRef title lookup, Jaccard confidence scoring, hard-error gating on low/unverified confidence in `article_download`

## Current State (v1.4 shipped)

**Shipped:** v1.4 DOI Verification — 2026-05-27
**Latest:** `article_download` cross-validates every DOI against CrossRef; low/unverified confidence returns `isError: true` — callers cannot silently receive wrong-paper results
**Codebase:** ~2,000 LOC TypeScript (src) + ~2,000 LOC (tests) ≈ 4,000 total
...
**Tests:** 114 passing across 10 test files
...
- ✓ CrossRef API client (`fetchCrossRefTitle`) retrieves canonical paper title by DOI with injectable fetch and 5s timeout — v1.4 phase 15
- ✓ Jaccard similarity confidence scoring (`computeConfidence`) on normalized word sets at 0.5 threshold — v1.4 phase 16
- ✓ `article_download` gates on DOI verification: high confidence returns `{ article, sources }`; low/unverified returns `isError: true` with descriptive message — v1.4 phase 17
- ✓ Batch `article_download` gates per-article: high → `{ doi, article, sources }`, low/unverified → `{ doi, error }` — v1.4 phase 17
- ✓ CrossRef unavailability absorbed via `.catch(() => null)` — server never crashes on third-party failure — v1.4 phase 17
...
| CrossRef fetch uses injected `fetchImpl` for testability | Same pattern as `lookupArticleByDoi` — full test isolation without network calls. | ✓ Good — v1.4 phase 15 |
| Hard-error gating model for low/unverified confidence | Post-phase design upgrade: callers cannot silently receive wrong-paper results; additive `verification` field replaced with `isError: true`. | ✓ Good — v1.4 phase 17 (post-phase upgrade by project owner) |
| Jaccard threshold 0.5 hardcoded | Sufficient signal for v1.4; configurable threshold deferred to avoid premature abstraction. | ✓ Good — revisit if false-positive rate becomes an issue |
...
---
*Last updated: 2026-05-28 — v1.4 milestone archived; hard-error gating model documented*
